...
Github. If you do a request using bad credentials, you will get a
`github.BadCredentialsException` error.

According to the PyGithub script where the Github class is defined, the global
timeout is set at 15 seconds. That is already long so don't bother extending.
Also, the there is a note in the script.
...
Note on paging and timeout.
    ...
    But the max might not always be good to use. Each query will be slower,
    even if it takes fewer queries.
